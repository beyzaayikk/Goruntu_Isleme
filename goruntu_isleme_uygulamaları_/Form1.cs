...
namespace goruntu_isleme_uygulamaları_
{
    public partial class Form1 : Form
    {
        ...
        static Bitmap GriDonusum(Bitmap GirisResmi)
        {
            ...
            //resimlerin bitmap ile tanımlanıp pictureBox'lara ayarlanması
            //for döngüleri her pikseli sırasıyla gezmeleri
        ...
        private void görüntüKırpmaToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            Bitmap image = new Bitmap(pictureBox1.Image);
            Bitmap kirp = resimKirp(image, new Rectangle(a, b, width, height));
            pictureBox2.SizeMode = PictureBoxSizeMode.StretchImage;
        ...
        private Color RGBtoHSV(Color rgb)
        {
            ...
            if (delta != 0) //delta değeri sıfıra eşit değilse 
            {
                if (max == r)
                    h = (g - b) / delta;
                else if (max == g)
        ...
        private void görüntüyeFiltreUygulanmasıBlurringToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Bitmap image = new Bitmap(pictureBox1.Image);
            int blurAmount = 5;
            Bitmap bulanik = bulaniklastir(image, blurAmount);
            pictureBox2.SizeMode = PictureBoxSizeMode.StretchImage;
            pictureBox2.Image = bulanik;
        }
        private Bitmap bulaniklastir(Bitmap image, int blurAmount)
        {
            Bitmap blurredImage = new Bitmap(image.Width, image.Height);

            // Her piksel için blur efekti uygula
            for (int x = 0; x < image.Width; x++)
            {
                for (int y = 0; y < image.Height; y++)
                {
                    Color blurredColor = CalculateBlurredColor(image, x, y, blurAmount);
        ...
        static Color CalculateBlurredColor(Bitmap image, int x, int y, int blurAmount)
        {
            int totalR = 0, totalG = 0, totalB = 0;
            int count = 0;

            // Çevredeki piksellerin rengini topla
        ...
        public Bitmap Goruntu_Uzaklastirma(Bitmap GirisResmi)
        {
            ...
            int ResimGenisligi = GirisResmi.Width;
            int ResimYuksekligi = GirisResmi.Height;
            Bitmap CikisResmi = new Bitmap(ResimGenisligi, ResimYuksekligi); // Küçültülmüş boyutları

            int x2 = 0, y2 = 0; //Çıkış resminin x ve y si olacak. 
            int KucultmeKatsayisi = 0.5;
        ...
        public Bitmap Goruntu_Yakinlastirma(Bitmap GirisResmi)
        {
            ...
            // Çıkış resminin boyutlarını iki katına çıkartın
            ...
            int BuyutmeKatsayisi = Convert.ToInt32(textBox2.Text);
        ...
        public static Bitmap HistogramGermesiYap(Bitmap GirisResmi)
        {
            ...
            // Giriş resminin en koyu ve en açık piksel değerlerini bul
            int minPixel = 255;
            int maxPixel = 0;
        ...
        public Bitmap ParlaklikArttir(Bitmap GirisResmi)
        {
            ...
            int parlaklikArrtirma = Convert.ToInt32(textBox3.Text);
        ...
        {
            Bitmap image = new Bitmap(pictureBox1.Image);
            Bitmap gri = Esikleme(image);
            pictureBox2.Image = gri;
            pictureBox2.SizeMode = PictureBoxSizeMode.StretchImage;
        }
        public Bitmap Esikleme(Bitmap GirisResmi)
        {

            int resimGenişliği = GirisResmi.Width;
            int resimYüksekliği = GirisResmi.Height;
            int eşiklemeDegeri = 155;
            ...
            for (int x = 0; x < resimGenişliği; x++)
            {
                ...
                {
                    Color okunanRenk = GirisResmi.GetPixel(x, y);
                    int ortalamaRenk = (okunanRenk.R + okunanRenk.G + okunanRenk.B) / 3;

                    if (ortalamaRenk >= eşiklemeDegeri)
                        çıkışResmi.SetPixel(x, y, Color.White); // Beyaz
                    else
                        çıkışResmi.SetPixel(x, y, okunanRenk); // Orijinal renk
                }
            }
        ...
        public static Bitmap GaussSaltPepper(Bitmap GirisResmi)
        {
            ...
            for (int i = 0; i < griResim.Height; i++)
            {
                for (int j = 0; j < griResim.Width; j++)
                {
                    if (noiseMask[i, j] == 0)
                    {
                        griResim.SetPixel(j, i, Color.Black); // Tuz pikseli
                    }
                    else if (noiseMask[i, j] == 20)
                    {
                        griResim.SetPixel(j, i, Color.White); // Biber pikseli
                    }
                }
            }

            // CikisResmi'ni griResim'e eşitle
            CikisResmi = griResim;
        ...
        {
            // Giriş görüntüsünün yolunu belirtin
            int ResimYuksekligi = GirisResmi.Height;
            int ResimGenisligi = GirisResmi.Width;
            Bitmap CikisResmi = new Bitmap(ResimGenisligi, ResimYuksekligi);

            // g(v) fonksiyonu için en düşük ve en yüksek piksel değerlerini bulma
            int minPixel = 255;
            int maxPixel = 0;
            ...
            for (int x = 0; x < ResimGenisligi; x++)
            {
                ...
                {
                    // Pikselin renk değerlerini alalım
                    Color pixelColor = GirisResmi.GetPixel(x, y);
                    int red = pixelColor.R;
                    int green = pixelColor.G;
                    int blue = pixelColor.B;

                    // Kırmızı, yeşil ve mavi bileşenlerinden en düşük ve en yüksek piksel değerlerini güncelleyelim
        ...
        public static Bitmap GenislemeIslemi(Bitmap GirisResmi)
        {
            ...
            for (int y = 0; y < ResimYuksekligi; y++)
            {
                for (int x = 0; x < ResimGenisligi; x++)
                {
                    ...
                    // Yapı elemanının her piksel konumunda kontrol et
